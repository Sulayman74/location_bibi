...
import { db, PROPERTY_CONFIG, FUNCTIONS_BASE_URL } from './firebase-config.js'
...
export async function loadGuestBooking(bookingId) {
  ...
  try {
    ...
    currentBooking = { id: bookingId, ...data, checkIn: toDate(data.checkIn), checkOut: toDate(data.checkOut) }
...
export function computeWifiAccess(booking) {
  ...
  const windowStart = new Date(checkIn.getTime()  - WIFI_WINDOW_HOURS * 3_600_000)
  const windowEnd   = new Date(checkOut.getTime() + WIFI_WINDOW_HOURS * 3_600_000)

  if (now < windowStart) {
    return { canAccess: false, phase: 'too-early', unlockAt: windowStart, msUntilUnlock: windowStart - now, checkIn }
  }
  if (now > windowEnd) {
...
export async function renderWifiSection(booking) {
  if (!booking) { showWifiLocked('Aucune réservation active.'); return }
  ...
  try {
    ...
    localStorage.setItem('villa_wifi', JSON.stringify({ ssid: wifiData.ssid, password: wifiData.password }))
    navigator.serviceWorker?.controller?.postMessage({
      type: 'CACHE_WIFI', ssid: wifiData.ssid, password: wifiData.password,
    })
  } catch (err) {
...
function showWifiUnlocked(ssid, password) {
  ...
  if (passEl) { passEl.dataset.realPassword = password; passEl.textContent = '••••••••' }
...
function startCountdown(targetDate, displayId, onComplete) {
  ...
  const update = () => {
    ...
    if (ms <= 0) { clearInterval(countdownInterval); onComplete?.(); return }
...
export function startCheckinCountdown(checkIn) {
  ...
  startCountdown(target, 'countdown-display', () => countdownEl.classList.add('hidden'))
}
...
export function renderStayInfo(booking) {
  ...
  set('stay-dates',  `${formatDateShort(checkIn)} → ${formatDateShort(checkOut)}`)
  set('stay-nights', `${nights} nuit${nights > 1 ? 's' : ''} · ${booking.guests || 1} voyageur${(booking.guests || 1) > 1 ? 's' : ''}`)
  set('guest-name-header', booking.guestName ? `Bonjour, ${booking.guestName.split(' ')[0]} !` : 'Espace voyageur')

  const badge = document.getElementById('stay-status-badge')
  if (badge) {
    const now = new Date()
    const ci  = new Date(checkIn);  ci.setHours(PROPERTY_CONFIG.checkInHour, 0, 0, 0)
    const co  = new Date(checkOut); co.setHours(PROPERTY_CONFIG.checkOutHour, 0, 0, 0)
    ...
    if (now < ci) {
      ...
      badge.className  = 'text-xs font-semibold px-2.5 py-1 rounded-full bg-stone-100 text-stone-500'
...
export function renderReviewSection(booking) {
  ...
  document.getElementById('google-review-link')?.setAttribute('href', googleLink)
  document.getElementById('post-google-review-link')?.setAttribute('href', googleLink)

  const airbnbLink = `https://www.airbnb.fr/users/${PROPERTY_CONFIG.airbnbListingId}/reviews`
  document.getElementById('airbnb-review-link')?.setAttribute('href', airbnbLink)
}
